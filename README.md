...
|---:|---|---|---|
| 1 | `OsLib` | `OsLibCore` | 4.2.11 coordinated foundation |
| 2 | `RaiUtils` | `RaiUtils` | 4.2.11 coordinated dependency line |
| 3 | `RaiImage` | `RaiImage` | 4.2.11 coordinated dependency line |
| 4 | `RaiDiagram` | `RaiDiagram` | 4.2.11: CR023 relationship rendering |
| 5 | `JsonPit` | `JsonPit` | 4.2.11: CR024 process-flag cleanup |
| 6 | `ImgSeeder` | `ImgSeeder` / `iorg` | 4.2.11 coordinated dependency line |
| 7 | `PitSeeder` | `PitSeeder` / `pits` | 4.2.11: CR024 clean CLI exit |
...
The prepared coordinated release is `4.2.11`. RAI starts the release chain
manually after reviewing the prepared commits and verification results.

RAIkeep 4.2.11 implements accepted CR023 PlantUML relationship rendering and
CR024 deterministic cleanup of owned PID-specific JsonPit process flags while
preserving the accepted CR022 CloudDrive invariant.

The principal functional changes are:

- `RaiDiagram`: renders Activity Interaction relationships with labels/cardinality and publishes its accepted compiler vocabulary.
...
- OsLibCore, RaiUtils, RaiImage, and ImgSeeder participate in the coordinated dependency line without behavioral changes.
...
Current coordinated 4.2.11 release notes:
...
scripts/release-chain.sh 4.2.11
```
...
2. Push that repository's requested version tag, such as `v4.2.11`, to trigger its publish workflow.
